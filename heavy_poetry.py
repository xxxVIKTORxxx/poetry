...
# Mutation functions for poetic word transformation
...
def mutate_consonant(word):
    ...
    if any(c in word for c in consonants):
        return re.sub(r'[bcdfghjklmnpqrstvwxyz]', random.choice(consonants), word, count=1)
    return word

def mutate_word_structure(word):
    if len(word) > 3:
        return word[::-1]  # Reverse the word for structural mutation
    return word
...
def mutate_for_rhythm(poem_lines):
    ...
    for line in poem_lines:
        ...
        mutated_line = []
        for word in words:
            mutation_choice = random.choice([mutate_vowel, mutate_consonant, mutate_word_structure])
            mutated_line.append(mutation_choice(word))
